...
static SD_HandleTypeDef uSdHandle[2];
HAL_SD_CardInfoTypedef uSdCardInfo[2];
...
unsigned char BSP_SD_Init(void)
{
	  ...
	  /* uSD device interface configuration */
	    uSdHandle[0].Instance = SDMMC2;
	    uSdHandle[0].Init.ClockEdge           = SDMMC_CLOCK_EDGE_RISING;
	    uSdHandle[0].Init.ClockBypass         = SDMMC_CLOCK_BYPASS_DISABLE;
	    uSdHandle[0].Init.ClockPowerSave      = SDMMC_CLOCK_POWER_SAVE_DISABLE;
	    uSdHandle[0].Init.BusWide             = SDMMC_BUS_WIDE_1B;
	    uSdHandle[0].Init.HardwareFlowControl = SDMMC_HARDWARE_FLOW_CONTROL_DISABLE;
	    uSdHandle[0].Init.ClockDiv            = SDMMC_TRANSFER_CLK_DIV;

	  /* Msp SD Detect pin initialization */
	  //BSP_SD_Detect_MspInit(&uSdHandle[0], NULL);
	  ...
	  if(HAL_SD_Init(&uSdHandle[0], &uSdCardInfo[0]) != SD_OK)
	  {
	    sd_state = MSD_ERROR;
	  }
	  ...
	  if(sd_state == MSD_OK)
	  {
	    ...
	    if(HAL_SD_WideBusOperation_Config(&uSdHandle[0], SDMMC_BUS_WIDE_4B) != SD_OK)
	    {
	      sd_state = MSD_ERROR;
	    }
	    else
...
{
  if(HAL_SD_ReadBlocks(&uSdHandle[0], pData, ReadAddr, BlockSize, NumOfBlocks) != SD_OK)
  {
    return MSD_ERROR;
  }
  else
...
{
  if(HAL_SD_WriteBlocks(&uSdHandle[0], pData, WriteAddr, BlockSize, NumOfBlocks) != SD_OK)
  {
    return MSD_ERROR;
  }
  else
...
unsigned char BSP_SD_ReadBlocks_DMA(unsigned long *pData, unsigned long long ReadAddr, unsigned long BlockSize, unsigned long NumOfBlocks)
{
  ...
  if(HAL_SD_ReadBlocks_DMA(&uSdHandle[0], pData, ReadAddr, BlockSize, NumOfBlocks) != SD_OK)
  {
    sd_state = MSD_ERROR;
  }
  ...
  if(sd_state == MSD_OK)
  {
    if(HAL_SD_CheckReadOperation(&uSdHandle[0], (unsigned long)SD_DATATIMEOUT) != SD_OK)
    {
      sd_state = MSD_ERROR;
    }
    else
...
unsigned char BSP_SD_WriteBlocks_DMA(unsigned long *pData, unsigned long long WriteAddr, unsigned long BlockSize, unsigned long NumOfBlocks)
{
  ...
  if(HAL_SD_WriteBlocks_DMA(&uSdHandle[0], pData, WriteAddr, BlockSize, NumOfBlocks) != SD_OK)
  {
    sd_state = MSD_ERROR;
  }
  ...
  if(sd_state == MSD_OK)
  {
    if(HAL_SD_CheckWriteOperation(&uSdHandle[0], (unsigned long)SD_DATATIMEOUT) != SD_OK)
    {
      sd_state = MSD_ERROR;
    }
    else
...
bool _mmcsd_idle(unsigned int unit_nr)
{
	...
	if(card_detect == NULL || card_detect->event.state_dn)
	{
		...
		if(card_detect != NULL)
			card_detect->event.state_dn = false;
        MmcSdFatFs.drv_rw_func.DriveStruct = (void *)&uSdCardInfo[0];//SdStruct;
