...
DRESULT USBH_read(BYTE lun, BYTE *buff, DWORD sector, UINT count)
{
  ...
  if ((DWORD)buff & 3) /* DMA Alignment issue, do single up to aligned buffer */
  {
    ...
    {
      status = USBH_MSC_Read(&hUSB_Host, lun, sector + count, (unsigned char *)scratch, 1);
      if(status == USBH_OK)
      {
...
{
	HCD_HandleTypeDef * _ctrl = (HCD_HandleTypeDef *)ctrl;
	  DRESULT res = RES_ERROR;
	  MSC_LUNTypeDef info;
	  unsigned char lun = 0;
	  switch (command)
...
void _usb_msc_host_idle(unsigned int instance)
{
    ...
    case APPLICATION_START:
        g_sFatFs1.drv_rw_func.DriveStruct = (void*)&hUSB_Host;
