...
#include "driver/stm32f4xx_sdio.h"
...
new_gpio *CardDetectPinMmcSd = NULL;
...
uint8_t SD_Detect(void)
{
  ...
  if (SDCardInfo.card_detect != NULL && gpio_in(SDCardInfo.card_detect) != Bit_RESET)
  {
...
{
	SD_CardInfo* _SdCtrlStruct = (SD_CardInfo*)SdCtrlStruct;
    switch(command)
    {
...
{
	SD_CardInfo *_SDCardInfo = (SD_CardInfo *)SdStruct;
	if(_SDCardInfo->status_led)
		gpio_out(_SDCardInfo->status_led, 1);
	SD_Error status = SD_OK;
	if(nblks == 1)
		SD_ReadBlock((uint8_t *)ptr, block, 512);
	else
		SD_ReadMultiBlocks((uint8_t *)ptr, block, 512, nblks);
	...
	if (status == SD_OK) {
		if(_SDCardInfo->status_led)
			gpio_out(_SDCardInfo->status_led, 0);
		return 1;
	} else {
		if(_SDCardInfo->status_led)
			gpio_out(_SDCardInfo->status_led, 0);
...
unsigned int MMCSDWriteCmdSend(void *SdStruct, void *ptr, unsigned long block, unsigned int nblks)
{
	...
	SD_Error status = SD_OK;
	if(nblks == 1)
		SD_WriteBlock((uint8_t *)ptr, block, 512);
	else
		SD_WriteMultiBlocks((uint8_t *)ptr, block, 512, nblks);
	/* Check if the Transfer is finished */
	status = SD_WaitWriteOperation();
	while(SD_GetStatus() != SD_TRANSFER_OK);
	if (status == SD_OK) {
...
{
	SD_CardInfo *_SDCardInfo = (SD_CardInfo *)SdStruct;
	if(_SDCardInfo->card_detect != NULL)
		gpio_idle(_SDCardInfo->card_detect);
	if(_SDCardInfo->card_detect == NULL || _SDCardInfo->card_detect->event.state_dn)
	{
		SD_Error res = SD_OK;
		res =  SD_Init();
		if(res != SD_OK)
		...
		if(_SDCardInfo->card_detect != NULL)
			_SDCardInfo->card_detect->event.state_dn = false;
		//mmcsd_init((void *)_SDCardInfo, 0, 0, _SDCardInfo->status_led);
        MmcSdFatFs.drv_rw_func.DriveStruct = (void *)_SDCardInfo;//SdStruct;
...
{
	SD_CardInfo *_SDCardInfo = (SD_CardInfo *)SdStruct;
	_SDCardInfo->status_led = StatusLed;
