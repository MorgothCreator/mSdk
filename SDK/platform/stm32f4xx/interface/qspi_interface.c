...
/******************************************************************************/
/******************************************************************************/
/*						Quad SPI Memory driver functions                      */
/******************************************************************************/
/******************************************************************************/
/******************************************************************************/
/* Private functions ---------------------------------------------------------*/
...
/** @defgroup STM32469I_Discovery_QSPI_Private_Functions QSPI Private Functions
  * @{
  */
static uint8_t QSPI_N24Q_ResetMemory          (Qspi_Mem_t *param);
static uint8_t QSPI_N24Q_DummyCyclesCfg       (Qspi_Mem_t *param);
static uint8_t QSPI_N24Q_WriteEnable          (Qspi_Mem_t *param);
static uint8_t QSPI_N24Q_AutoPollingMemReady  (Qspi_Mem_t *param, uint32_t Timeout);
...
uint8_t QSPI_N24Q_Read(Qspi_Mem_t *param, uint8_t* pData, uint32_t ReadAddr, uint32_t Size)
{
  ...
  s_command.DummyCycles       = param->dev_info->DumyCiclesReadQuad;
...
uint8_t QSPI_N24Q_Write(Qspi_Mem_t *param, uint8_t* pData, uint32_t WriteAddr, uint32_t Size)
{
  ...
  {
    current_addr += param->dev_info->PageSize;
  }
  ...
  do
  {
    ...
    if (QSPI_N24Q_WriteEnable(param) != QSPI_OK)
    {
      return QSPI_ERROR;
    }
    ...
    if (QSPI_N24Q_AutoPollingMemReady(param, HAL_QPSI_TIMEOUT_DEFAULT_VALUE) != QSPI_OK)
    {
      return QSPI_ERROR;
    }
    ...
    pData += current_size;
    current_size = ((current_addr + param->dev_info->PageSize) > end_addr) ? (end_addr - current_addr) : param->dev_info->PageSize;
...
uint8_t QSPI_N24Q_Erase_Block(Qspi_Mem_t *param, uint32_t BlockAddress)
{
  ...
  if (QSPI_N24Q_AutoPollingMemReady(param, param->dev_info->SubSectorEraseMaxTime) != QSPI_OK)
  {
    return QSPI_ERROR;
  }
...
uint8_t QSPI_N24Q_Erase_Chip(Qspi_Mem_t *param)
{
  ...
  /* Configure automatic polling mode to wait for end of erase */
  if (QSPI_N24Q_AutoPollingMemReady(param, param->dev_info->BulkEraseMaxTime) != QSPI_OK)
  {
    return QSPI_ERROR;
  }
...
uint8_t QSPI_N24Q_GetStatus(Qspi_Mem_t *param)
{
  ...
  if ((reg & (FSR_PRERR | FSR_VPPERR | FSR_PGERR | FSR_ERERR)) != 0)
  {
    return QSPI_ERROR;
  }
  else if ((reg & (FSR_PGSUS | FSR_ERSUS)) != 0)
  {
    return QSPI_SUSPENDED;
  }
  else if ((reg & FSR_READY) != 0)
  {
    return QSPI_OK;
  }
  else
...
/**
  ...
  * @retval None
  */
__weak void QSPI_N24Q_MspInit(void *Params)
{
...
/**
  ...
  * @retval None
  */
__weak void QSPI_N24Q_MspDeInit(void *Params)
{
...
static uint8_t QSPI_N24Q_DummyCyclesCfg(Qspi_Mem_t *param)
{
  ...
  MODIFY_REG(reg, VCR_NB_DUMMY, (param->dev_info->DumyCiclesReadQuad << POSITION_VAL(VCR_NB_DUMMY)));
