...
#define HOST_OVRCURR_PIN_SOURCE            GPIO_PinSource11
#define HOST_OVRCURR_PORT_RCC              RCC_APB2Periph_GPIOD
#define HOST_OVRCURR_EXTI_LINE             EXTI_Line11
#define HOST_OVRCURR_IRQn                  EXTI15_10_IRQn 

/*detect the OTG ID pin state */
#define OTG_HS_ID_PORT_RCC                RCC_AHB1Periph_GPIOE
#define OTG_HS_ID_PORT                    GPIOE
#define OTG_HS_ID_LINE                    GPIO_Pin_2
#define OTG_HS_ID_EXTI_PORT_SOURCE        EXTI_PortSourceGPIOE
#define OTG_HS_ID_EXTI_PIN_SOURCE         EXTI_PinSource2
#define OTG_HS_ID_EXTI_LINE               EXTI_Line2
#define OTG_HS_ID_EXTI_IRQn               EXTI2_IRQn 

/*OTG Host power control pin state */
#define OTG_HS_PWR_PORT_RCC               RCC_AHB1Periph_GPIOA
#define OTG_HS_PWR_PORT                   GPIOA
#define OTG_HS_PWR_PIN                    GPIO_Pin_8
...
void USB_OTG_BSP_Init(USB_OTG_CORE_HANDLE *pdev)
{
  ...
  /* Configure pin for OTG_HOST detection */
  RCC_AHB1PeriphClockCmd(OTG_HS_ID_PORT_RCC, ENABLE);
  /* Reset the GPIO_InitStructure variable */
  /* Configure ID Pin */
  GPIO_InitStructure.GPIO_Pin   = OTG_HS_ID_LINE ; 
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
  GPIO_InitStructure.GPIO_Mode  = GPIO_Mode_IN;
  GPIO_InitStructure.GPIO_PuPd  = GPIO_PuPd_UP ;
  GPIO_Init(OTG_HS_ID_PORT, &GPIO_InitStructure);

  /* Configure pin for OTG Power Manager */
  RCC_AHB1PeriphClockCmd(OTG_HS_PWR_PORT_RCC, ENABLE);
  /* Configure Power Pin */
  GPIO_InitStructure.GPIO_Pin   = OTG_HS_PWR_PIN ; 
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
  GPIO_InitStructure.GPIO_Mode  = GPIO_Mode_OUT;
  GPIO_InitStructure.GPIO_PuPd  = GPIO_PuPd_UP ;
  GPIO_Init(OTG_HS_PWR_PORT, &GPIO_InitStructure);												    
#endif //USB_OTG_HS
...
/**
  * @brief  Get_OTG_HS_ID_State
  *         
  * @param  None
  * @retval None
  */
uint8_t Get_OTG_HS_ID_State(void)
{
  return GPIO_ReadInputDataBit(OTG_HS_ID_PORT, OTG_HS_ID_LINE);
}


/**
  * @brief  Enable_OTG_HS_PWR
  *         
  * @param  None
  * @retval None
  */
void Enable_OTG_HS_PWR(void)
{
  GPIO_SetBits(OTG_HS_PWR_PORT, OTG_HS_PWR_PIN);
}

/**
  * @brief  Disable_OTG_HS_PWR
  *         
  * @param  None
  * @retval None
  */
void Disable_OTG_HS_PWR(void)
{
  GPIO_ResetBits(OTG_HS_PWR_PORT, OTG_HS_PWR_PIN);
}
