...
void I2CAINTCUnConfigure(new_twi* TwiStruct)
{
    ...
    case 0:
    	IntProtectionDisable();
        /* Enabling the system interrupt in AINTC. */
        IntSystemDisable(SYS_INT_I2C1_IRQ);

        /* Registering the Interrupt Service Routine(ISR). */
        IntUnRegister(SYS_INT_I2C1_IRQ);
        IntProtectionEnable();
    	break;
    case 1:
    	IntProtectionDisable();
        /* Enabling the system interrupt in AINTC. */
    	IntSystemDisable(SYS_INT_I2C2_IRQ);

    	/* Registering the Interrupt Service Routine(ISR). */
    	IntUnRegister(SYS_INT_I2C2_IRQ);
        IntProtectionEnable();
    	break;
    case 2:
    	IntProtectionDisable();
        /* Enabling the system interrupt in AINTC. */
    	IntSystemDisable(SYS_INT_I2C3_IRQ);

    	/* Registering the Interrupt Service Routine(ISR). */
    	IntUnRegister(SYS_INT_I2C3_IRQ);
        IntProtectionEnable();
    	break;
    case 3:
    	IntProtectionDisable();
        /* Enabling the system interrupt in AINTC. */
    	IntSystemDisable(SYS_INT_I2C4_IRQ);

    	/* Registering the Interrupt Service Routine(ISR). */
    	IntUnRegister(SYS_INT_I2C4_IRQ);
        IntProtectionEnable();
    	break;
    }
}
