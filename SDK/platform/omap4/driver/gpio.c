...
{
	GPIO_t *base_add = (GPIO_t*)baseAdd;
	base_add->GPIO_SYSCONFIG |= GPIO_SYSCONFIG_SOFTRESET_bm;
	while(!(base_add->GPIO_SYSSTATUS & GPIO_SYSSTATUS_RESETDONE_bm));
}
...
void GPIOModuleEnable(unsigned int baseAdd)
{
	...
	GPIO_t *base_add = (GPIO_t*)baseAdd;
	base_add->GPIO_CTRL &= ~(GPIO_CTRL_DISABLEMODULE_bm);
}
                            ...
                            unsigned int pinNumber)
{
	...
	GPIO_t *base_add = (GPIO_t*)baseAdd;
    if(!(base_add->GPIO_OE & (1 << pinNumber)))
    {
                  ...
                  unsigned int pinValue)
{
    ...
    if(pinValue)
    {
    	base_add->GPIO_SETDATAOUT = (1 << pinNumber);
    }
    else
    {
    	base_add->GPIO_CLEARDATAOUT = (1 << pinNumber);
    }
}
...
{
	GPIO_t *base_add = (GPIO_t*)baseAdd;
    return base_add->GPIO_DATAIN & (1 << pinNumber);
}
                           ...
                           unsigned int clrMask)
{
	...
	base_add->GPIO_SETDATAOUT = setMask;

    /* Clearing the specified output pins in GPIO_DATAOUT register. */
	base_add->GPIO_CLEARDATAOUT = clrMask;
}
...
{
	GPIO_t *base_add = (GPIO_t*)baseAdd;
    return base_add->GPIO_DATAIN & readMask;
}
...
{
	GPIO_t *base_add = (GPIO_t*)baseAdd;
    contextPtr->ctrl = base_add->GPIO_CTRL;
    contextPtr->dir = base_add->GPIO_OE;
}
...
{
	GPIO_t *base_add = (GPIO_t*)baseAdd;
	base_add->GPIO_CTRL = contextPtr->ctrl;
	base_add->GPIO_OE = contextPtr->dir;
}
