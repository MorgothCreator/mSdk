 ...
 *
 * This file contains the _main buffer controller for decompression.
 * The _main buffer lies between the JPEG decompressor proper and the
 * post-processor; it holds downsampled data in the JPEG colorspace.
 *
 * Note that this code is bypassed in raw-data mode, since the application
 * supplies the equivalent of the _main buffer in that case.
 */
...
/*
 * In the current system design, the _main buffer need never be a full-image
 * buffer; any full-height buffers will be found inside the coefficient or
 * postprocessing controllers.  Nonetheless, the _main controller is not
 ...
 */
{
  my_main_ptr _main = (my_main_ptr) cinfo->_main;
  int ci, rgroup;
 ...
 * The actual workspace is already allocated (in _main->buffer),
			   ...
			   JDIMENSION out_rows_avail)
{
  ...
  case CTX_PREPARE_FOR_IMCU:
    ...
    _main->rowgroup_ctr = 0;
    _main->rowgroups_avail = (JDIMENSION) (cinfo->min_DCT_v_scaled_size - 1);
  ...
  case CTX_PROCESS_IMCU:
    ...
    _main->rowgroup_ctr = (JDIMENSION) (cinfo->min_DCT_v_scaled_size + 1);
    _main->rowgroups_avail = (JDIMENSION) (cinfo->min_DCT_v_scaled_size + 2);
...
/*
 * Initialize _main buffer controller.
 */
...
jinit_d_main_controller (j_decompress_ptr cinfo, boolean need_full_buffer)
{
  ...
  cinfo->_main = (struct jpeg_d_main_controller *) _main;
